...
def get_Bass_Pro_Shop_company_document() -> list[str]:
    ...
    try:
        ...
        for e in entities:
            if e["entity"][0] == "B":
                labels.append(e["word"])
            else:
                labels[-1] = labels[-1] + ' ' + e["word"]
        results = collection.query(query_texts=[query], n_results=2, where={"organization": labels[0]})
...
def get_George_going_to_dinner_document() -> list[str]:
    ...
    try:
        ...
        ner_pipe = create_ner_pipeline()
        raw_entities = ner_pipe(query)
        mixed = []
        entity = {}
        for i in range(len(raw_entities)):
            if raw_entities[i]["entity"] == "B-PER":
                mixed.append(["person", [raw_entities[i]["word"]]])
            elif raw_entities[i]["entity"] == "I-PER":
                mixed[-1][1].append(raw_entities[i]["word"])
            elif raw_entities[i]["entity"] == "B-LOC":
                mixed.append(["location", [raw_entities[i]["word"]]])
            elif raw_entities[i]["entity"] == "I-LOC":
                mixed[-1][1].append(raw_entities[i]["word"])
        ...
        results = collection.query(
            ...
            n_results=2, 
            where={
                "$and": [{key:value} for key, value in entity.items()]
            }
        )   
        return [results["ids"][0], results["documents"][0]]
...
def get_George_Washington_Document() -> list[str]:
    ...
    try:
        ...
        ner_pipe = create_ner_pipeline()
        entities_split = ner_pipe(query)
        entities_grouped = []
        for i in range(len(entities_split)):
            if entities_split[i]["entity"] == "B-PER":
                entities_grouped.append([f"person_{i}", [entities_split[i]["word"]]])
            elif entities_split[i]["entity"] == "I-PER":
                entities_grouped[-1][1].append(entities_split[i]["word"])
        entities = {}
        for e in entities_grouped:
            entities[e[0]] = " ".join(e[1])
                        
        results = collection.query(
            query_texts=[query], 
            n_results=2, 
            where={
                "$and": [{value:True} for value in entities.values()]
            }
        )
        return [results["ids"][0], results["documents"][0]]
